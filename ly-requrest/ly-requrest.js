...
class Request {
	...
	ajax({
		...
		return new Promise((resolve, reject) => {
			...
			let requestTask = uni.request({
				...requestInfo, //请求的参数
				complete: (rtnInfo) => {
					// console.log(requestInfo,rtnInfo) //请求后的信息
					// 状态不等于200说明有错误
					if (rtnInfo.statusCode != 200) {
						...
						switch (true) {
							case isRemoveNull && typeof data != 'object' || data == null: // 判断是否是开启null转换并且传入的值类型是普通数据类型
								//请求后置拦截  + Null处理
								afterAjax ? resolve(afterAjax(removeNull(rtnInfo.data))) : resolve(defaultData.afterAjax(removeNull(
									rtnInfo.data)));
								break;
					...
					if (rtnInfo.errMsg.indexOf("ok") == -1) {
						...
						let uploadTask = uni.uploadFile({
							...
							complete: (rtnInfo) => {
								...
								if (rtnInfo.statusCode != 200) {
									// 错误拦截
									error ? error(errInfo(rtnInfo, "upload"), reject) : defaultData.error(errInfo(rtnInfo, "upload"),
										reject);
								} else {
									//请求后置拦截
									afterAjaxUpload ? resolve(afterAjaxUpload(rtnInfo.data)) : resolve(defaultData.afterAjaxUpload(
										rtnInfo.data));
									// 如果outTitle值存在，就显示提示，outTitle就是显示的值
									if (outTitle)(uni.showToast({
										title: outTitle
						...
						//上传前置拦截
						beforeAjaxUpload ? beforeAjaxUpload(uploadInfo, uploadTask) : defaultData.beforeAjaxUpload(uploadInfo,
							uploadTask)
					}
				}
			})
	 ...
	 * @param {Object} header 请求头信息
	 * @param {Object} beforeAjaxDownload 前置拦截
	 * @param {Object} afterAjaxDownload 后置拦截
	 * @param {Function(Event)} error 错误回调
	 * @returns {Promise}  返回一个Promise对象 路径
	 */
	ajaxDownload({
		title = false,
		outTitle = false,
		...
		return new Promise((resolve, reject) => {
			...
			let downloadTask = uni.downloadFile({
				...
				complete: (rtnInfo) => {
					...
					if (rtnInfo.statusCode != 200) {
						// 错误拦截
						error ? error(errInfo(rtnInfo, "download"), reject) : defaultData.error(errInfo(rtnInfo, "download"),
							reject);
					} else {
						//请求后置拦截
						afterAjaxDownload ? resolve(afterAjaxDownload(rtnInfo.tempFilePath)) : resolve(defaultData.afterAjaxDownload(
							rtnInfo.tempFilePath));
						// 如果outTitle值存在，就显示提示，outTitle就是显示的值
						if (outTitle)(uni.showToast({
							title: outTitle
						}));
			...
			//下载前置拦截
			beforeAjaxDownload ? beforeAjaxDownload(downloadInfo, downloadTask) : defaultData.beforeAjaxDownload(downloadInfo,
				downloadTask);
