/**
 * @property { object } defaultData 默认值
 * @property { boolean } defaultData.isRemoveNull - 是否需要把null转为字符串""
 * @property { object } defaultData.baseUrl - 基础地址
 * @property { object } defaultData.data - 传递的参数
 * @property { string } [defaultData.method=GET] - 请求方式
 * @property { object } defaultData.header - 请求头
 * @property { string } defaultData.dataType - 返回数据格式
 * @property { object } defaultData.dataPublic - 每次请求带上的参数
 * @property { ['album','camera'] } defaultData.sourceType - album 从相册选图，camera 使用相机，默认二者都有。如需直接开相机或直接选相册，请只使用一个选项
 * @property { object } defaultData.formData - HTTP 请求中其他额外的 form data
 * @property { function } defaultData.beforeAjax - 请求前置拦截
 * @property { function } defaultData.afterAjax - 请求后置拦截
 * @property { function } defaultData.beforeAjaxUpload - 上传前置拦截
 * @property { function } defaultData.afterAjaxUpload - 上传后置拦截
 * @property { function } defaultData.beforeAjaxDownload - 下载前置拦截
 * @property { function } defaultData.afterAjaxDownload - 下载后置拦截
 * @property { function } defaultData.error - 错误拦截
 *
 */
const defaultData = {
...
class Request {
	 ...
	 * @param {String} [data.title] - 是否需要显示loading加载提示框 title值就是loading的值
	 * @param {String} [data.outTitle] - 是否需要显示加载成功提示框 outTitle值就是提示的值
	 * @param {String} data.path - 请求的地址
	 * @param {Object} [data.data] - 请求带过去的参数 默认为空对象
	 * @param { string } [data.method] - 请求的方式
	 * @param {Object} [data.header] - 请求头信息
	 * @param {String} [data.dataType] - 默认json返回的信息默认为json会尝试进行一次json解析
	 * @param {Function} [data.beforeAjax] - 请求前置拦截回调
	 * @param {Function} [data.afterAjax] - 请求后置拦截回调
	 * @param {Function} [data.error] - 错误回调
	 * @return {Promise} - 返回一个Promise请求对象
	 */
	ajax({
		title = false,
		outTitle = false,
	 ...
	 * @param { String } data.path - 上传的地址
	 * @param { String } data.name - 文件对应的 key , 开发者在服务器端通过这个 key 可以获取到文件二进制内容。
	 * @param { Object } [data.header] - 请求头信息
	 * @param { Object } [data.formData] - HTTP请求中其他额外的formData
	 * @param { Array } [data.sourceType] - album从相册选图,camera使用相机，默认二者都有。如需直接开相机或直接选相册，请只使用一个选项
	 * @param { Function } [data.beforeAjaxUpload] - 上传前置拦截回调
	 * @param { Function } [data.afterAjaxUpload] - 上传后置拦截回调
	 * @param { Function } [data.error] - 错误回调
	 ...
	 * @param { String } [data.title] - 是否需要显示loading加载提示框 title值就是loading的值
	 * @param { String } [data.outTitle] - 是否需要显示加载成功提示框 outTitle值就是提示的值
	 * @param { String } data.path - 下载的地址
	 * @param { Object } [data.header] - 请求头信息
     * @param { function } [data.beforeAjaxDownload] - 上传前置拦截
     * @param { function } [data.afterAjaxDownload] - 上传后置拦截
	 * @param { Function } [data.error] - 错误回调
	 * @returns { Promise } - 返回一个Promise对象
	 */
	ajaxDownload({
