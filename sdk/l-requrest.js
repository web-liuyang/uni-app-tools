...
/**
 * @property { Boolean } isRemoveNull - 是否需要把null转为字符串""
 * @property { String } baseUrl - 基础地址
 * @property { Object } data - 传递的参数
 * @property { Boolean } method = [GET|POST] - 默认请求方式
 * @property { String } dataType = [json] - 默认返回数据为JSON格式
 * @property { Object } dataPublic - 默认请求时带的公共参数 常用于设置token
 * @property { String[] } sourceType = [album|camera] - album 从相册选图，camera 使用相机，默认二者都有。如需直接开相机或直接选相册，请只使用一个选项
 * @property { Object } formData - HTTP 请求中其他额外的 form data
 * @property { Function } beforeAjax - 请求前置拦截
 * @property { Function } afterAjax - 请求后置拦截
 * @property { Function } beforeAjaxUpload - 上传前置拦截
 * @property { Function } afterAjaxUpload - 上传后置拦截
 * @property { Function } beforeAjaxDownload - 下载前置拦截
 * @property { Function } afterAjaxDownload - 下载后置拦截
 * @property { Function } error - 错误拦截
 */
const defaultData = {
...
/**
 *  @param {*} data 需要传入的去除null值的对象或者值 递归函数
 *  @param {*} defaultStr 将null值转为该字符串, 不传默认为 空字符串 ''
 */
...
class Request {
   ...
   * @param { String } data.method = [GET|POST] - 请求的方式
   * @param { Object } data.header - 请求头信息 content-type':"application/x-www-form-urlencoded
